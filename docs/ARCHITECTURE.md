...
- **The sim is unit-testable headless.** Over 850 assertions across 17 test files run in a few seconds
...
| `src/ai/` | Producer | `src/sim/`, `src/arena/`, `Tuning` | `bot_controller.gd` — FSM and difficulty; `grid_path.gd` — A* over the arena grid; `remote_controller.gd` — a person on another phone, which is the same kind of thing (ADR-0032) |
...
| `src/sim/snapshot.gd` | Simulation | `Tuning`, `FighterClass` | The wire format, pure and static. In `src/sim/` rather than `src/net/` because a snapshot is a DESCRIPTION OF WORLD STATE; that ENet usually carries one is the network layer's business (ADR-0032) |
| `src/net/` | Transport + bridge | `src/sim/`, `Tuning` | `net_link.gd` — host, join, peers, application-level ping; `lan_beacon.gd` — UDP broadcast discovery; `net_game.gd` (autoloaded as **`Lan`**) — roster, seats, and the two RPC paths |
...
| ~~**The LAN spike has never been runnable**~~ | ~~The overlay builds Tuning, Log and Info only; nothing in `main.gd` touches `NetLink`~~ **Five milestones of "awaiting real hardware" with no build that could ask** | **PAID.** `feat/lan` built the lobby, the Net tab, and the netcode the spike was deferring to. Three of its four questions are answered in code; the rest need a router |
| **Nobody has run this on two phones** | Loopback proves the whole round trip at 7 ms — command → authority → snapshot → view — which is every part of the path except the radio. Whether this router carries it, whether broadcast discovery survives it, and what the real latency is are unknown | The phone. It is the one gate this environment cannot be made to run, and [LAN_SPIKE.md](LAN_SPIKE.md) says what to tap and what to read back |
| **No client-side prediction** | At LAN latency, invisible. At 200 ms your own cat answers your thumb a fifth of a second late and the game feels broken. Assumed rather than measured, by an explicit decision to avoid a spike PR ahead of a working game | Contained if it bites: movement-only prediction in `src/net/net_game.gd`, against a game that already works. The DBG Net tab's ping is what decides |
| **A dropped phone becomes a bot** | No reconnection. Someone whose screen locks mid-match loses the round | Unclaimed, and arguably correct for this audience: nobody sits out and nothing stalls waiting for them |
