...
| ~~Web export is a debug build (36 MB wasm)~~ | **The premise was false.** The release export measured **bigger** — 40.1 MB vs 38.5 MB on disk, identical gzipped (10.4 MB) | **CLOSED, NOT PAID.** Switching would also have deleted the DBG panel from the web channel, since it gates on `OS.is_debug_build()` |
